  ...
  try {
    ...
    async function updateSlippageTolerance() {
      ...
      if (newSlippageTolerance) {
        if (isNaN(newSlippageTolerance) || newSlippageTolerance < 0 || newSlippageTolerance > 10) {
          console.error("Invalid slippage tolerance. It must be a number between 0 and 10%.");
        } else {
          const confirmChange = await askQuestion(
